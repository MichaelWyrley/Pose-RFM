# Taken from https://github.com/moonbow721/DPoser/blob/v2/lib/body_model/joint_mapping.py
import os
...
def mmpose_to_openpose(keypoints, keypoint_scores):
    ...
    # print(keypoints.shape, keypoint_scores.shape)
    # assert len(keypoints) == 133, f"Expected 133 keypoints, got {len(keypoints)}"
    keypoints = torch.cat((keypoints, keypoint_scores.unsqueeze(-1)), dim=2)
...
def vitpose_to_openpose(keypoints):
    ...
    # assert len(keypoints[0]) == 133, f"Expected 133 keypoints, got {len(keypoints)}"
    ...
    # Call merge_joint with the prepared parts
    merged_joints = merge_joint(body_kpts, feet_kpts, left_hand_kpts, right_hand_kpts, face_kpts)
    idx_mapping = torch.Tensor(coco_to_openpose())
...
def merge_joint(joint_img, feet_img, lhand_img, rhand_img, face_img):
    ...
    neck = neck.reshape(-1, 1, 3)

    # feet
    lfoot = feet_img[:, :3, :]
    rfoot = feet_img[:, 3:, :]

    joint_img = torch.cat((joint_img, pelvis, neck, lfoot, rfoot, lhand_img, rhand_img, face_img), dim = 1) # [135, 3]
    return joint_img
