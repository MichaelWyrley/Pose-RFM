...
class Posendf(nn.Module):
    ...
    def forward(self, pose, betas, *args):
        pose_quat = transforms.axis_angle_to_quaternion(pose.view(-1, 21, 3))
...
class Pose_RFM(nn.Module):
    ...
    def forward(self, pose, betas, *args):
        # Denoise pose
        predicted_pose = self.model.denoise_pose(pose, args['initial_timestep'], args['timesteps'], args['scale'])
        # Calculate pose loss as distance traveled for denoising
        optimise_mx = transforms.euler_angles_to_matrix(pose, "XYZ").reshape(-1,3,3)
