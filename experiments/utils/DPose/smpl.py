...
from experiments.utils.DPose.constants import SMPL_MEAN_PATH, SMPL_REGRESSOR_PATH
from experiments.utils.DPose.transforms import rot6d_to_axis_angle
...
# chumpy uses an older version of numpy, this is a work arround to fix compatability issues
np.bool, np.int, np.float, np.complex, np.object, np.unicode, np.str, np.nan, np.inf = np.bool_, np.int_, np.float_, np.complex_, np.object_, np.unicode_, np.str_, np.nan, np.inf
...
class SMPL(_SMPL):
    ...
    def __init__(self, *args, **kwargs):
        ...
        J_regressor_extra = np.load(SMPL_REGRESSOR_PATH)
        smpl_mean_params = np.load(SMPL_MEAN_PATH)
