...
from Utils.NRDF.utils.data_utils import geo, load_faiss,quat_to_global, cal_intrinsic_geo
...
def pose_to_vert(pose_body, args, device='cuda'):
    ...
    bm = BodyModel(bm_fname, num_betas=num_betas, model_type='smplh').to(pose_body.device)
...
def gen_partial(diffusion, args):
    ...
    if args['gen_masks']:
        ...
        np.savez(args['directory'] + args['save_location'] + 'mask.npz', pose_body=masks.cpu().numpy())
    else: 
        np.load(args['directory'] + args['save_location'] + 'mask.npz')

    total_APD = []
    total_FID = []
    total_dnn = []

    cdata = np.load(args['directory'] + args['dataset_data'])
    dataset_mu, dataset_std = cdata['mu'], cdata['std']

    
    # load faiss related terms
    index, all_poses_aa, all_poses_quat = load_faiss(args['directory'] + args['faiss_model'])
