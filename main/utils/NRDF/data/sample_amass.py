...
def sample_amass_frames(data_dir, mode='train'):
    in_dir = osp.join(data_dir, 'RAW_DATA')
    out_dir = osp.join(data_dir, 'SAMPLED_FRAMES')
    os.makedirs(out_dir, exist_ok=True)
    ...
    for split in data_splits:
        ...
        for seq in seqs:
            ...
            for npz_fname in npz_fnames:
                ...
                cdata_ids = np.linspace(int(0.1 * pose_len), int(0.9 * pose_len), 1)

                if len(cdata_ids) < 1:
                    continue

                full_pose = cdata['poses'][cdata_ids].astype(np.float64)
                pose_body.extend(full_pose[:, 3:72])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_root', '-d', type=str, default='/vol/bitbucket/mew23/individual-project/dataset/amass',
                        help='Data path')
    parser.add_argument('--mode', '-m', type=str, default='valid', help='AMASS train/test/valid subset')
