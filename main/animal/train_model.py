...
from main.animal.dataloader import PoseData
...
def train_multigpu_loop(rank, world_size, args):
    ...
    data_set = PoseData(mode='train', clean_dir=args['clean'], batch_size=args['batch_size'], num_workers=6, stage=1, flip=False, random_poses=False)
    ...
    for epoch in range(0, args['epochs']+1):
        ...
        if epoch % 50 == 0 and rank==0:
            torch.save(model.module.state_dict(), f"./samples/training_animal_models/model_{epoch}.pt")
            torch.save(ema_model.module.state_dict(), f"./samples/training_animal_models/ema_model_{epoch}.pt")
