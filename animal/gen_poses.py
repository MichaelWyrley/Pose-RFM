...
from vis_animal import visualise
...
def sample_single(diffusion, labels,args):
    ...
    if labels is None:
        ...
        np.savez(args['directory'] + args['sample_dir'] + f"sample_0", pose_body=pose_body, categories= np.zeros(args['samples'], dtype=int))
    else:
        ...
        np.savez(args['directory'] + args['sample_dir'] + f"sample_0", pose_body=pose_body, categories= labels)
...
def sample_full_for_video(diffusion, labels,args):
    ...
    for i in range(args['sample_timestep']):
        ...
        np.savez(args['directory'] + args['sample_dir'] + f"sample_{i}", pose_body=pose_body, categories= labels)
        if args['print']: print(f"Finished with {i}")
...
if __name__ == '__main__':
    args = {
        ...
        'scale': 6,
        'sample_dir': 'animal/samples/',   
        'frame': 'animal/samples/sample_0.npz',
        'print': True,
