...
class Posendf(nn.Module):
    def __init__(self, config, ckpt_path, num_joints=21):
        super().__init__()
    ...
    def forward(self, pose, betas, *args):
        reshaped_pose = pose[:, :self.num_joints*3].view(-1, self.num_joints, 3)

        pose_quat = axis_angle_to_quaternion(reshaped_pose.view(-1, 21, 3))
...
class NRDF(nn.Module):
    def __init__(self, config, ckpt_path, num_joints=21, device='cuda'):
        super().__init__()
        sys.path.insert(0, './other_priors')
        from NRDF.lib.model.nrdf import NRDF
        from NRDF.configs.config import load_config

        model = NRDF(load_config(config))
    ...
    def forward(self, pose, betas, *args):
        ...
        reshaped_pose = pose[:, :self.num_joints*3].view(-1, self.num_joints, 3)
        noisy_pose_quat = axis_angle_to_quaternion(reshaped_pose) # [bs, nj, 4]

        dist_pred = self.model(noisy_pose_quat, train=False)['dist_pred']
        return torch.mean(dist_pred) * 1e2

class VPoser(nn.Module):
    def __init__(self, support_dir, num_joints=21):
        super().__init__()
    ...
    def forward(self, pose, betas, *args):
        ...
        coding = self.vposer.encode(reshaped_pose).mean
        l2_norm = torch.norm(coding, p=2, dim=[1], keepdim=True)
        return torch.mean(l2_norm) * 0.5

class Pose_RFM(nn.Module):
    def __init__(self, ckpt_path, timestep= 15, initial_timestep=10, scale=1, num_joints=21, device='cuda'):
        super().__init__()
        sys.path.insert(0, '../Pose-RFM')
        ...
        self.model = FlowMatchingMatrix(model, device=device)
    ...
    def forward(self, pose, betas, *args):
        ...
        predicted_pose = self.model.denoise_pose(reshaped_pose, self.initial_timestep, self.timestep, self.scale)
        # Calculate pose loss as distance traveled for denoising
        optimise_mx = transforms.axis_angle_to_matrix(reshaped_pose).reshape(-1,3,3)
        predicted_mx = transforms.axis_angle_to_matrix(predicted_pose).reshape(-1,3,3)
        pose_loss = transforms.so3_relative_angle(optimise_mx, predicted_mx).reshape(predicted_pose.shape[0], predicted_pose.shape[1])
