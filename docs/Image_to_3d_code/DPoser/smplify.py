...
# Please also see the specific priors as to where to save any files that are needed for the priors to work
# FOR POSE-RFM: this code assumes that DPoser and it are saved in the same directory, change as needed
...
class SMPLify:
    ...
    def __init__(self,
        ...
        if args.prior == 'DPoser':
            ...
            from lib.body_model.prior import VPoser
            support_dir = './other_priors/human_body_prior/support_data/downloads'
            ...
            config = './other_priors/PoseNDF/checkpoints/config.yaml'
            ckpt = './other_priors/PoseNDF/checkpoints/checkpoint_epoch_best.tar'
            self.pose_prior = Posendf(config, ckpt).to(self.device)
        elif args.prior == 'NRDF':
            from lib.body_model.prior import NRDF
            config = './other_priors/NRDF/amass_softplus_l1_0.0001_10000_dist0.5_eik0.0_man0.1/config.yaml'
            ckpt = './other_priors/NRDF/amass_softplus_l1_0.0001_10000_dist0.5_eik0.0_man0.1/checkpoints/checkpoint_epoch_best.tar'
            self.pose_prior = NRDF(config, ckpt).to(self.device)
        elif args.prior == 'Pose-RFM':
            from lib.body_model.prior import Pose_RFM
            ckpt_path = '../Pose-RFM/best_model/ema_model_1200.pt'
            self.pose_prior = Pose_RFM(ckpt_path, device=self.device).to(self.device)
        else:
