...
def partial_generation(diffusion, args):
    ...
    with torch.no_grad():
        for i, poses in enumerate(data_loader):
            ...
            np.savez(args['directory'] + f'./samples/missing_points/generated/data_{i}.npz', pose_body=partial_pose)
            np.savez(args['directory'] + f'./samples/missing_points/original/data_{i}.npz', pose_body=clean_pose)
            break
