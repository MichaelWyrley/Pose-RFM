...
amass_splits = {
    ...
    'practice': ['CMU', 'MPI_Limits', 'TotalCapture', 'Eyes_Japan_Dataset', 'KIT', 'BioMotionLab_NTroje' , 'EKUT', 'TCD_handMocap']
}
...
def log_map(a, b):
    ...
    a_inv_b = quaternion_hamilton_product(a_conj, b)
    w = a_inv_b[..., 0]
    v = a_inv_b[..., 1:]
    ...
    result = torch.zeros_like(a, device=a.device)
    result[..., 1:] = torch.nn.functional.normalize(v, dim=2) * theta.unsqueeze(-1)
    return result

def exp_map(a,b, epsilon=0.000001):
    w = b[..., 0]
    v = b[..., 1:]
    theta = torch.linalg.norm(v, dim=2)

    new_w = torch.cos(theta)
    new_v = (v * ((torch.sin(theta) / (theta+epsilon)).unsqueeze(-1)))
    result = torch.cat((new_w.unsqueeze(-1), new_v), dim=2)

    return quaternion_hamilton_product(a, result)
