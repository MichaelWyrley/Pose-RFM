...
from Utils.NRDF.utils.data_utils import quaternion_hamilton_product, amass_splits
...
class Pose_Noise_Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        poses = quaternion_to_axis_angle(torch.Tensor(poses))
        ...
        clean_poses = torch.Tensor(clean_poses.reshape(-1, 21, 3))
        
        return poses, clean_poses
...
class PoseData(Dataset):
    def __init__(self, mode, clean_dir, batch_size=4, num_workers=6, num_pts=5000, stage=1, flip=False,
                 random_poses=False):
        self.mode = mode
        self.clean_dir = clean_dir
        self.num_pts = num_pts
...
class FrameData(Dataset):
    def __init__(self, mode, clean_dir, batch_size=4, num_workers=6, seq_len_max=100, stage=1, flip=False,
                 random_poses=False):
        self.mode = mode
        self.clean_dir = clean_dir
    ...
    def __getitem__(self, idx):
        ...
        if ((len(cdata['pose_body']) - self.seq_len_max) <= 0):
            start_frame = 0
        else: 
            start_frame = np.random.randint(0, len(cdata['pose_body']) - self.seq_len_max)

        clean_poses = cdata['pose_body'][:, :63][start_frame:start_frame+self.seq_len_max].astype(np.float32)
